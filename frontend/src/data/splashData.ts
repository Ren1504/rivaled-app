// Auto-generated mapping database of heroes and their downloaded skin names
export interface HeroSplashData {
  [heroName: string]: string[];
}

export const heroSplashData: HeroSplashData = {
  "Adam Warlock": [
    "Blood Soul.png",
    "Cosmic Warlock.png",
    "Default.png",
    "Guardians of The Galaxy Vol. 3.png",
    "Immortal Avatar.png",
    "Living Tribunal.webp",
    "Magus.png"
  ],
  "Angela": [
    "Ace Of Spades.png",
    "Default.png",
    "Odin's Beautiful Daughter.png",
    "Skuld 2099.png"
  ],
  "Black Cat": [
    "Default.webp",
    "Urban Predator.png",
    "Wild Card.png"
  ],
  "Black Panther": [
    "Bast's Chosen.png",
    "Damisa-sarki.png",
    "Default.png",
    "Galactic Claw.png",
    "King Of Wakanda.png",
    "Thrice-cursed King.png"
  ],
  "Black Widow": [
    "Aquatic Assassin.png",
    "Default.png",
    "Lion's Heartbeat.png",
    "Midnight Suspense.png",
    "Mrs. Barnes.png",
    "Red Runway Veil.png",
    "White Suit.png"
  ],
  "Blade": [
    "Default.png",
    "Knight.png",
    "Polarity Edge.png",
    "Restful Recovery.png",
    "Starlit Shadow.png"
  ],
  "Captain America": [
    "Avengers- Infinity War.png",
    "Brett Hendrick- The Star.png",
    "Captain Gladiator.png",
    "Captain Klyntar.png",
    "Capwolf.png",
    "Cursed Captain.png",
    "Default.png",
    "Galactic Talon.png",
    "Primal Patriot.png",
    "Star Spangled Style.png"
  ],
  "Cloak & Dagger": [
    "Balance of Nature.png",
    "DEFAULT.png",
    "Duality Duet.png",
    "Mafia Attack.png",
    "NO REGRETS.png",
    "SHADOWFLAME DUET.png",
    "WINTER REALM WALTZ.png"
  ],
  "Cyclops": [
    "Weapon M.png"
  ],
  "Daredevil": [
    "Daredevil- Born Again Season 2.png",
    "Default.png",
    "Devil 2099.png",
    "Not Daredevil.png",
    "Shadowed Start.png",
    "Sonar Saviour.png"
  ],
  "Deadpool": [
    "Captain Pool.png",
    "Maid-pool.png",
    "Monster King.png"
  ],
  "Devil Dinosaur": [
    "Chrono-armor.png"
  ],
  "Doctor Strange": [
    "Bleeker Street Strut.png",
    "Default.png",
    "God Of Magic.png",
    "In The Multiverse Of Madness.png",
    "Nautical Mystic.png",
    "Phantom Sorcerer.png",
    "Sorcerer Immortal.png",
    "Sorcerer Supreme Of The Galaxy.png",
    "Stephen Sanders- The Doctor.png"
  ],
  "Elsa Bloodstone": [
    "Bloody Bloodstone.png",
    "Young Blood.png"
  ],
  "Emma Frost": [
    "Black Queen Of The Marauders.png",
    "Default.png",
    "Gala Glam.png",
    "Hellfire Protocol.png",
    "Phoenix Diamond.png",
    "Queen Of Diamonds.png",
    "White Wave Queen.png",
    "X-revolution.png"
  ],
  "Gambit": [
    "Alluring Ace.png",
    "Barrier Buster.png",
    "Default.png",
    "Mr. X.png",
    "Thieves Guildmaster.png"
  ],
  "Groot": [
    "Default.png",
    "Guardians Of The Galaxy Vol. 3.png",
    "Holiday Happiness.png",
    "Ironwood Serpent.png",
    "Mecha-flora.png",
    "Yggroot.png"
  ],
  "Hawkeye": [
    "Default.png",
    "Freefall.png",
    "Galactic Fangs.png",
    "Old Man Hawkeye.png",
    "Ronin.png"
  ],
  "Hela": [
    "Default.png",
    "Disco Of The Dead.png",
    "Empress Of The Cosmos.png",
    "Goddess Of Death.png",
    "Helen Angerboda- The Rebel.png",
    "Queen In Black.png",
    "Savage Monarch.png",
    "The Grim Lady.png",
    "Yami No Karasu.png"
  ],
  "Hulk": [
    "Default.png",
    "Green Scar.png",
    "Infinity Hulk.png",
    "Joe Fixit.png",
    "Maestro.png",
    "Punk Rage.png"
  ],
  "Human Torch": [
    "Default.png",
    "Jack Of Hearts.png",
    "Negative Zone Gladiator.png",
    "Sunny Sizzler.png"
  ],
  "Invisible Woman": [
    "Azure Shade.png",
    "Default.png",
    "Disappearing Dessert.png",
    "Future Foundation.png",
    "Malice.png",
    "Prism Parade.png",
    "Riann Landau- The Debater.png",
    "The Life Fantastic.png"
  ],
  "Iron Fist": [
    "Binary Fist.png",
    "Default.png",
    "Immortal Weapon Of Agamotto.png",
    "Lin Shao- The Knockout.png",
    "Lion's Gaze.png",
    "Phoenix Fist.png",
    "Sword Master.png"
  ],
  "Iron Man": [
    "Avengers- Endgame.png",
    "Big Shot.png",
    "Blood Edge Armor.png",
    "Default.png",
    "Extrembiote Armor.png",
    "Iron Mariner.png",
    "Iron Samurai.png",
    "Mark I.png",
    "Steam Power.png",
    "Superior Iron Man.png",
    "The Avengers.png"
  ],
  "Jeff the Land Shark": [
    "Business Shark.png",
    "Cuddly Fuzzlefin.png",
    "Default.png",
    "Devil Landshark.png",
    "Devouring Duo.png",
    "Duck Defender.png",
    "Incognito Dolphin.png",
    "Jeff O'lantern.png",
    "Maid To Chomp.png",
    "Sunshine Land Shark.png"
  ],
  "Loki": [
    "All-butcher.png",
    "Default.png",
    "Immortal Firebird.png",
    "Lady Loki.png",
    "Low-key Loki.png",
    "Presidential Attire.png",
    "Robe Of Relaxation.png",
    "Season 2.png",
    "Shin Sagi-shi.png",
    "The Avengers.png"
  ],
  "Luna Snow": [
    "Abyssal Glow.png",
    "Cherry Delight.png",
    "Cool Summer.png",
    "Default.png",
    "Disco Pop.png",
    "Mirae 2099.png",
    "Park Sun-young- The Dancer.png",
    "Shining Star.png",
    "Sonic Trailblazer.png"
  ],
  "Magik": [
    "Default.png",
    "Eldritch Armor.png",
    "Frozen Demon.png",
    "Infernal Idol.png",
    "Netherworld Noble.png",
    "New Millennia Might.png",
    "Phoenix Demon.png",
    "Punkchild.png",
    "Soulless Sword.png"
  ],
  "Magneto": [
    "Binary Sword.png",
    "Default.png",
    "King Magnus.png",
    "Master Of Magnetism.png",
    "Seat Of Autumn.png",
    "Temporal Tyrant.png",
    "The Trial Of Magneto.png",
    "Untamed Overlord.png"
  ],
  "Mantis": [
    "Default.png",
    "Galactic Gladiator.png",
    "Galactic Wings.png",
    "Guardians Of The Galaxy Vol. 3.png",
    "Jade Maiden.png",
    "Mandy Celestine- The Greeter.png",
    "Oceanic Harmony.png"
  ],
  "Mister Fantastic": [
    "Dad-tastic Reed.png",
    "Danum Ket- The Professor.png",
    "Default.png",
    "Fantastic Beach Dad.png",
    "Future Foundation.png",
    "The Life Fantastic.png",
    "The Maker.png"
  ],
  "Moon Knight": [
    "Default.png",
    "Eclipse Knight.png",
    "Fist Of Vengeance.png",
    "King Of Clubs.png",
    "Lunar General.png",
    "Mech.png",
    "Mister Knight.png",
    "Phoenix Knight.png",
    "Suave Spector.png"
  ],
  "Namor": [
    "Black Panther- Wakanda Forever.png",
    "Default.png",
    "Monstro King.png",
    "Phantom Tide.png",
    "Phoenix King.png",
    "Retro X-uniform.png",
    "Sea Samba.png"
  ],
  "Peni Parker": [
    "Badlands Lullaby.png",
    "Default.png",
    "Floral Frights.png",
    "Peni Parker- The Freshman.png",
    "Snow-sp--dr.png",
    "Ven#m.png",
    "Wasteland Mech.png",
    "Yatsukahagi.png"
  ],
  "Phoenix": [
    "Chaos Phoenix.png",
    "Dark Phoenix.png",
    "Default.png",
    "Ice Phoenix.png",
    "Seat Of Spring.png",
    "Verdant Vogue.png",
    "White Crown Phoenix.png"
  ],
  "Psylocke": [
    "Blood Kariudo.png",
    "Breezy Butterfly.png",
    "Chouko Takahashi- The Prodigy.png",
    "Daring Daifuku.png",
    "Default.png",
    "Fleeting Butterfly.png",
    "Once-captive Corsair.png",
    "Psychedelic Pulse.png",
    "Vengeance.png"
  ],
  "Rocket Raccoon": [
    "Bounty Hunter.png",
    "Default.png",
    "Giant Panda.png",
    "Guardians Of The Galaxy Vol. 3.png",
    "Rocket Of The Rafters.png",
    "Sunshine Raccoon.png",
    "Symbiote Raccoon.png",
    "Wild Winter.png"
  ],
  "Rogue": [
    "Default.png",
    "Mrs. X.png",
    "Redux.png",
    "Starlit Rebel.png"
  ],
  "Scarlet Witch": [
    "Default.png",
    "Doctor Strange In The Multiverse Of Madness.png",
    "Emporium Matron.png",
    "Immortal Sovereign.png",
    "Phoenix Chaos.png",
    "The Queen Of The Dead.png",
    "Twisted Conjurer.png",
    "Witch Of The Evil Eye.png"
  ],
  "Spider-Man": [
    "Bag-man Beyond.png",
    "Black Suit.png",
    "Coastal Wall Crawler.png",
    "Default.png",
    "Future Foundation.png",
    "Groovy Swing.png",
    "Man-spider.png",
    "Marvel Cosmic Invasion.png",
    "Peter Parker- The Photographer.png",
    "Spider-man- No Way Home.png",
    "Spider-oni.png",
    "Spider-punk 2099.png"
  ],
  "Squirrel Girl": [
    "Cheerful Dragoness.png",
    "Default.png",
    "Maureen Allene- The Programmer.png",
    "Nut Rocker.png",
    "Red Panda.png",
    "Sunshine Squirrel.png",
    "Symbiote Squirrel.png",
    "Tinsel Tail.png",
    "Turbo Tailwind.png",
    "Urban Hunter.png"
  ],
  "Star-Lord": [
    "Default.png",
    "Glory Gunner.png",
    "Groovy Guardian.png",
    "Guardians Of The Galaxy Vol. 3.png",
    "King Of Spartax.png",
    "Lion's Mane.png",
    "Master Of The Sun.png",
    "Starcracker.png",
    "Starlit Outlaw.png"
  ],
  "Storm": [
    "Default.png",
    "Goddess Of Thunder.png",
    "Judicator Xiezhi.png",
    "Mohawk Rock.png",
    "Symbiote Storm.png",
    "Ultimate Wind-rider.png"
  ],
  "The Punisher": [
    "Aqua Arsenal.png",
    "Dangan Ronin.png",
    "Daredevil- Born Again.png",
    "Default.png",
    "Franken-castle.png",
    "Marauder Marksman.png",
    "Punisher 2099.png"
  ],
  "The Thing": [
    "Default.png",
    "Fear Itself.png",
    "Future Foundation.png",
    "Rocky Tide.png",
    "Symbiote-thing.png",
    "Trench Coat.png"
  ],
  "Thor": [
    "Boogie Bolt.png",
    "Default.png",
    "God Of Winter.png",
    "Herald Of Thunder.png",
    "Lightning Fast.png",
    "Lord Of Asgard.png",
    "Low-key Thor.png",
    "Majestic Raiment.png",
    "Thor- Love And Thunder.png",
    "Unworthy Thor.png",
    "Worthy Waves.png"
  ],
  "Ultron": [
    "Default.png",
    "Infinity Ultron.png",
    "Odinforce Ultron.png",
    "Wasteland Robot.png",
    "X-tron.png"
  ],
  "Venom": [
    "Default.png",
    "Gummy Surprise.png",
    "Lingering Imprint.png",
    "Marvel Cosmic Invasion.png",
    "Reborn King In Black.png",
    "Snow Symbiote.png",
    "Space Corsair.png",
    "Space Knight.png",
    "War Of The Realms.png"
  ],
  "White Fox": [
    "Default.png",
    "Yoon Ji-woo- The Leader.png"
  ],
  "Winter Soldier": [
    "Blood Soldier.png",
    "Default.png",
    "Polarity Soldier.png",
    "Revolution.png",
    "Starlit Gunslinger.png",
    "Thunderbolts-.png",
    "Winter Buckaroo.png"
  ],
  "Wolverine": [
    "Blood Berserker.png",
    "Deadpool & Wolverine.png",
    "Default.png",
    "Dog Brother X.png",
    "Patch.png",
    "Weapon X.png",
    "Winter Soldier.png",
    "X-2099.png"
  ]
};
